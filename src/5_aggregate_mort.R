...
file.agg <- paste0(path.agg, region, ".mort.", pred_type, ".rds")
...
merge.cols <- c(id.var, "mort", "year")
data <- data[, ..merge.cols]
data[, type := factor(pred_type)]

data <- 
  bin_cols(data, "mort", 5, 0, bin.names = "mort.bin5", append = TRUE, right = FALSE) |>
  bin_cols("mort", 10, 0, bin.names = "mort.bin10", append = TRUE, right = FALSE)

group.sel <- c("group.id", "type", "mort", "mort.bin5", "mort.bin10", "year")
group.by <- list("mort",
                 c("year", "mort"),
                 "mort.bin5",
                 c("year", "mort.bin5"),
